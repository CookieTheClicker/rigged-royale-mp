...
(function () {
  const SERVER_URL = window.RIGGED_SERVER_URL || 'http://localhost:3000';
...
`;
  ...
  elements.joinCode.addEventListener('input', () => {
    elements.joinCode.value = elements.joinCode.value.toUpperCase().replace(/[^A-Z0-9]/g, '').slice(0, 6);
  });
  ...
  elements.readyToggle.addEventListener('change', () => {
    if (!checkSocket()) return;
    state.socket.emit('party:ready', { ready: elements.readyToggle.checked });
  });
  ...
  function cleanName(value) {
    return String(value || '').replace(/[^a-zA-Z0-9 _-]/g, '').trim().slice(0, 20);
  }
  ...
  function setPartyMessage(text, type = 'info') {
    ...
    elements.partyMessage.style.color = type === 'error' ? '#ff9a9a' : '#f8c471';
  ...
  function updatePartyUI() {
    ...
    const members = hasParty && Array.isArray(state.party.members) ? state.party.members : [];
    const maxSize = hasParty ? state.party.maxSize : 0;

    elements.partyCode.textContent = hasParty ? `Code: ${state.party.code}` : 'Not in a party';
    elements.partyMeta.textContent = hasParty ? `${members.length}/${maxSize} players` : '';
    ...
    if (hasParty) {
      members.forEach((member) => {
        ...
        pieces.push(member.name || (member.id ? member.id.slice(0, 5) : 'player'));
  ...
  function initSocket() {
    ...
    socket.on('connect_error', (err) => {
      setPartyMessage(`Connection error: ${err && err.message ? err.message : 'unknown'}`, 'error');
    });
  ...
  script.onerror = () => setPartyMessage('Failed to load multiplayer client', 'error');
